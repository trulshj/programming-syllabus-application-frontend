...
const  ArticlesList= ({match}) => {
    ...
    function getTimeDiff(inputTime){
        ...
        if(mmsDiff>24*60*60*1000){
            const diffDays=(time.getTime()-inputTime.getTime())/(24*60*60*1000)
            return diffDays.toString().split(".")[0] +" dager siden";
        }else if(mmsDiff>60*60*1000){
            const diffDays=(time.getTime()-inputTime.getTime())/(60*60*1000)
    ...
    useEffect( () => {
         async function fetchData() {
             ...
             if(window.location.pathname==="/articlelist/myarticles"){
                 //@ts-ignore
                 newArticles=await fetchArticlesByUser(localStorage.getItem("userId"));
            ...
            <CardColumns >
                    {articles ? (articles.map((article, key) => (
                        <Card className ="ArticlesList" key={key}>
                            {
                                // @ts-ignore
                                <Card.Img variant={"top"} src={article?.images["0"]?"https://localhost:8080/api/file/"+article.images["0"].file_id:ntnu} alt={article?.images["0"]?article.images["0"].alt_text:"ingen bilder for dette undervisningsopplegget"}/>
                            }
                        <Card.Body>
                        <Card.Title>{article.article_title.toString().substr(0,maxTitleLenght)+" ..."}</Card.Title>
                        <Card.Text>
                            {article.article_description.toString().substr(0,maxDescriptionLenght)+" ..."}
                        </Card.Text>
                        <Link to={`/articlelist/${article.article_id}`}>
                        <Button variant="primary">Gå til egenside</Button>
                        </Link>
                        </Card.Body>
                        <Card.Footer>
                        <small className="text-muted">Sist oppdatert {getTimeDiff(new Date(article.article_change_date))}</small>
                        </Card.Footer>
                    </Card>
